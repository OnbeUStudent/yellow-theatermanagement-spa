...
namespace FakeTheaterBff.Features.SeedData
{
  public static class DataSeeding
  {
    ...
    public static void SeedData(FakeTheaterBffContext context)
    {
      ...
      const string theatercode_synthcinemas = "synthcinemas";
      ...
      DetailedTheater[] detailedTheaters = {
                ...
                new DetailedTheater()
                {
                    TheaterCode = theatercode_synthcinemas,
                    TheaterName = "Synthetic Cinemas",
                    AddressLine1 = "123 Synthia St",
                    City = "Synthton",
                    State = "CA",
                    Zip = "90210",
                    ThemeType = ThemeType.Cyborg
                }
            };
      ...
      for (int i = 0; i < months.Count && i < movies.Count; i++)
      {
        ...
        // Book synthcinemas with the movies in reverse chronological order
        var synthcinemasBooking = new Booking()
        {
          TheaterCode = theatercode_synthcinemas,
          MonthId = months[i].MonthId,
          MovieId = movies[movies.Count - 1 - i].MovieId
        };
        if (!context.Bookings.Any(b => b.MonthId == synthcinemasBooking.MonthId && b.TheaterCode == theatercode_synthcinemas))
        {
          context.Add(synthcinemasBooking);
        }
      }
      context.SaveChanges();
    }
